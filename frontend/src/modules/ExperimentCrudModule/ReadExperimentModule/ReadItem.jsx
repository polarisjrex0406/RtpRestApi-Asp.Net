...
const TopicPromptItem = ({ order, item, currentErp }) => {
...
export default function ReadItem({ config, selectedItem }) {
  ...
  return (
    <>
      ...
      <Divider />
      <Descriptions title='Topic Prompts' />
      {currentErp?.initPrompt?.map((item, index) => (
        <TopicPromptItem key={'topicPrompt' + index} order={index + 1} item={item} currentErp={currentErp} />
      ))}
