...
export default function Card({ data, isLoading = false, onClickViewTemplates, onClickRead, onClickEdit, onClickDelete, onClickDuplicate }) {
  ...
  }, [data.group, data.goal, data.promptOutput]);
      ...
      >
        ...
        <div className="pad15">
          <Row gutter={[0, 0]} justify="space-between" wrap={false}>
            <Col className="gutter-row" flex="85px" style={{ textAlign: 'left' }}>
              <div className="left" style={{ maxWidth: `${maxWidth}px`, whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }} ref={divRef}>
                Prompt: {!isLoading ? data?.promptOutput : ''}
              </div>
            </Col>
          </Row>
        </div>
