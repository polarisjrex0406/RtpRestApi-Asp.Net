...
import { Dropdown, Table, Button, Input, Divider, Spin } from 'antd';
...
export default function CardBox({ config, extra = [] }) {
  ...
  return (
    <>
      ...
      {(() => {
        const cards = [];
        let pastTopicId = null;
        for (const i in dataSource) {
          if (dataSource[i]?.topicId != pastTopicId) {
            cards.push(<Divider dashed key={`${uniqueId()}`} />);
            cards.push(<Descriptions key={`${uniqueId()}`} title={`${dataSource[i]?.topic?.name}`}></Descriptions>)
            pastTopicId = dataSource[i]?.topicId;
          }
          cards.push(<Card
            key={`${uniqueId()}`}
            data={dataSource[i]}
